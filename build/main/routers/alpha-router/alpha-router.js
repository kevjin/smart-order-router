...
class AlphaRouter {
  ...
  async route(
    ...
    if (
      ...
      if (
        tradeType === sdk_core_1.TradeType.EXACT_INPUT &&
        (this.chainId === chains_1.ChainId.MAINNET ||
          this.chainId === chains_1.ChainId.GÖRLI) &&
        /// The cases where protocols = [] and protocols = [V2, V3, MIXED]
        (protocolsSet.size == 0 ||
          protocolsSet.has(router_sdk_1.Protocol.MIXED))
      ) {
        log_1.log.info(
  ...
  async applyTokenValidatorToPools(pools, isInvalidFn) {
    ...
    const poolsFiltered = lodash_1.default.filter(pools, (pool) => {
      ...
      if (token0Invalid || token1Invalid) {
        log_1.log.info(
          `Dropping pool ${(0, routes_1.poolToString)(
            pool
  ...
  async getMixedRouteQuotes(
    ...
    for (const routeWithQuote of routesWithQuotes) {
      ...
      for (let i = 0; i < quotes.length; i++) {
        ...
        const routeWithValidQuote =
          new route_with_valid_quote_1.MixedRouteWithValidQuote({
            route,
            rawQuote: quote,
            amount,
            percent,
